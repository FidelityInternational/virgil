...
// GetRuntimeVMs - Returns an array of Deployment VMs for runtime components (DEA's/ Diego Cells) for a deployment
func (c *Client) GetRuntimeVMs(deploymentName string) (DeploymentVMs, error) {
	...
	deploymentVMs := sanitiseRuntimeVMs(allDeploymentVMs)
...
func sanitiseRuntimeVMs(allDeploymentVMs DeploymentVMs) DeploymentVMs {
	var deploymentVMs DeploymentVMs
	for _, deploymentVM := range allDeploymentVMs {
		deaMatched, _ := regexp.MatchString("^dea-partition.+", deploymentVM.JobName)
		diegoMatched, _ := regexp.MatchString("^diego_cell-partition.+", deploymentVM.JobName)
		if deaMatched || diegoMatched {
